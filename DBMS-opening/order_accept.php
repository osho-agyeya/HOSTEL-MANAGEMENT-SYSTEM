...
<body>
    <div id="wrapper">
        ...
        <div id="overview">
            <center>
            <h1><b><font size=6 ><u>Your Orders</font></u></b></h1>
<?php
	$regno=$_SESSION['id'];
	$query2="select * from orders where reg_no='".$regno."' order by num desc;";
	$r=mysql_query($query2) or die('not execute');
	$r1=mysql_query("select * from orders;");
	$nums=mysql_num_rows($r1);
	$i=$nums;
echo "<form action='' method='post'>";
	...
	if($row['flag']=='PENDING')
	{
		...
		echo "<div class='container1' id='signup'>
							  <div class='header'>
            				  <h3>Please Collect Your Order</h3>
        					  </div>
        					  <div class='sep'></div>
							  <p>Registration number:$row[reg_no] &nbsp &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Name:$row[name_stud]</p>
		                      <p>Address:$row[address] &nbsp &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
		                      	 Room:$row[room_stud]</p>
		                      <p>DOA:$row[doa]</p>
                              <input type='submit' class='myB' name='accept$row[num]' value='accept'/>
              </div>";
	}
	else{
		echo "<div class='container1' id='signup'>
...
for($j=0;$j<=$nums;$j++)
{
	if(isset($_POST["accept$j"]))
	{
		...
		$query2="update orders set flag='ACCEPTED' where num=$j;";
		mysql_query($query2);
